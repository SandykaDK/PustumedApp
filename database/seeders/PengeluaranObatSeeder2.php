...
class PengeluaranObatSeeder2 extends Seeder
{
    ...
    'Plester Coklat / Putih' => [1, 10, 9, 2, 14, 2, 15, 9, 11, 12, 2, 3, 2, 6, 14, 8, 2, 11, 3, 14, 12, 12, 3, 2, 14, 1, 6, 7, 7, 8],
            ...
            while ($currentDate->lte($endDate)) {
                ...
                $pengeluaran = PengeluaranObat::create([
                    ...
                    'user_id' => $userIds[$dayIndex % count($userIds)],
                    'pasien_id' => $pasienIds[$dayIndex % count($pasienIds)],
                    'dokter_id' => $dokterIds[$dayIndex % count($dokterIds)],
                    'keterangan' => 'Pengeluaran obat bulan ' . $bulan . ' untuk tanggal ' . $currentDate->format('d F Y'),
                ]);
                ...
                foreach ($dailyUsagePatterns as $namaObatNama => $pattern) {
                    ...
                    $jumlahKeluar = $pattern[$dayIndex % count($pattern)];
                    if ($jumlahKeluar <= 0) {
                        continue;
                    }

                    $stok = $stokByNamaObatId->get($namaObat->id)?->last();
